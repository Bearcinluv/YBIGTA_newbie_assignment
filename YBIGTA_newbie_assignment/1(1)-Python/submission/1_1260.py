...
class Graph:
    ...
    def dfs(self, start: int) -> list[int]:
        ...
        # 구현하세요!
        visited = []
        visited_check = [False] * (self.n + 1)
        def _dfs_recursive(node: int):
            visited_check[node] = True
            visited.append(node)
            for next_node in sorted(self.graph[node]):
                if not visited_check[next_node]:
                    _dfs_recursive(next_node)
    ...
    def bfs(self, start: int) -> list[int]:
        ...
        visited_check = [False] * (self.n + 1)
        queue = deque([start])
        visited_check[start] = True
        visited.append(start)

        while queue:
            current = queue.popleft()
            for next_node in sorted(self.graph[current]):
                if not visited_check[next_node]:
                    visited_check[next_node] = True
