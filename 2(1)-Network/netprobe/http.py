...
def parse_status_and_preview(raw: bytes, max_preview: int = 200) -> tuple[Optional[int], str, Optional[str]]:
    ...
    if sep_index == -1:
        error_msg = "Invalid HTTP response: No header-body separator found."
        return None, "", error_msg

    header = raw[:sep_index].decode("utf-8", errors="replace")
    body = raw[sep_index + len(separator):]
    
    # 2. 첫 줄(Status Line)에서 상태 코드(int)를 추출합니다. [cite: 115]
    ...
    try:
        ...
        error_msg = "Invalid HTTP response: Malformed status line."
        return None, "", error_msg

    # 3. 바디 영역을 max_preview 만큼 decode하여 preview를 만듭니다. [cite: 116]
